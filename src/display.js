...
let currentProject = savedProjectIndex
  ? projects[savedProjectIndex]
  : projects[0];
...
const displayProjectHeader = () => {
  ...
  if (projects.length === 0) {
    ...
    newH2.textContent = `Project: ${getCurrentProject().name}`;
...
const toggleAddProjectBtn = () => {
  addProjectBtn.style.visibility =
    addProjectBtn.style.visibility === 'hidden' ? 'visible' : 'hidden';
};
